...
int main(){
    ...
    assert(chain_get(c, "Salut", &"Test") ==false);
    chain_set(c, "Chat", "Miaou");
    chain_set(c, "Chien", "Waf");
    chain_set(c, "Vache", "Meuh");
    chain_set(c, "Poule", "Cot Cot");
    chain_print(c);
    ...
    assert(chain_get(c, "Chien", &"Waf") ==false);
    chain_print(c);
    ...
    assert(hash_get(h, "Salut", &"Test") ==false);
    hash_set(h, "Chat", "Miaou");
    hash_set(h, "Chien", "Waf");
    hash_set(h, "Vache", "Meuh");
    hash_set(h, "Poule", "Cot Cot");
    hash_print(h);
    printf("-----\n");
    ...
    hash_delete(h, "Chien");
    assert(hash_get(h, "Chien", &"Waf") ==false);
    hash_print(h);
    hash_free(h);
    free(v2);
