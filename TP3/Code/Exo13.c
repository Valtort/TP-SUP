...
#include <time.h>
#include <stdlib.h>
/* Ce programme a pour but d'exploiter les "Cents mille milliards de poèmes"
de Raymond Queneau. L'ouvrage est un sonnet (14 vers), chaque vers possède
10 versions différentes interchangeables. Il y a ainsi 10^14 combinaisons possibles
*/
...
int main(){
	srand(time(NULL));
    for(int i=0; i<14;i++){
        int random = rand()%10; // Chiffre aléatoire entre 0 et 9
        printf("%s\n", vers[i][random]);
        if(i==3 || i==7 || i==9 ){ // Pour avoir un vrai affichage de Sonnet.
            printf("\n");
        }
    }
    return 0;
